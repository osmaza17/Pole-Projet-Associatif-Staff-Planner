...
class MandatoryRulesManager:
    ...
    def open_dialog(self, e, edit_index=None):
        ...
        def _rebuild_preview(ps, pidx, all_panels):
            ...
            hrs   = ps["sel_hours"]
            if not persons:
                ps["preview_text"].value = "Select one or more persons to begin."
                ps["preview_text"].color = ft.Colors.GREY_500
            else:
                names_str = ", ".join(sorted(persons))
                if len(persons) > 3:
                    names_str = ", ".join(sorted(persons)[:3]) + f" (+{len(persons)-3})"
                if jrest:
                ...
                if hour_parts:
                    ps["preview_text"].value = (
                        f"{names_str} → {task_str} — {'; '.join(hour_parts)}")
                    ps["preview_text"].color = (
                        "#B71C1C" if jrest else self._BTN_COLOR)
                else:
                    ps["preview_text"].value = f"{names_str} selected — choose mode and hours."
        ...
        def _rebuild_person_chips(ps, pidx, all_panels):
            ...
            selected = ps["persons"][0]
            sections = []

            # ── Select-all / deselect-all bar ──────────────────────────
            def _sel_all_persons(ev, _ps=ps, _pidx=pidx):
                for _gn, _members in s.build_groups(people).items():
                    for _p in _members:
                        _ps["persons"][0].add(_p)
            ...
            def _clr_all_persons(ev, _ps=ps, _pidx=pidx):
                _ps["persons"][0].clear()
                _rebuild_person_chips(_ps, _pidx, panels)
                _rebuild_task_chips(_ps, _pidx, panels)
                _rebuild_hours_section(_ps, _pidx, panels)
            ...
            sections.append(ft.Row([
                ft.Text(f"{sel_count} selected", size=_s(9),
                        color=ft.Colors.GREY_600, italic=True),
                ft.TextButton("Select all", on_click=_sel_all_persons,
                    style=ft.ButtonStyle(padding=ft.padding.symmetric(
                        horizontal=_s(6), vertical=_s(2)))),
                ft.TextButton("Clear", on_click=_clr_all_persons,
            ...
            for g_idx, (gname, members) in enumerate(groups.items()):
                ...
                def _sel_group(ev, _members=visible, _ps=ps, _pidx=pidx):
                    for _p in _members:
                        _ps["persons"][0].add(_p)
                    _rebuild_person_chips(_ps, _pidx, panels)
                    _rebuild_task_chips(_ps, _pidx, panels)
                ...
                def _clr_group(ev, _members=visible, _ps=ps, _pidx=pidx):
                    for _p in _members:
                        _ps["persons"][0].discard(_p)
                    _rebuild_person_chips(_ps, _pidx, panels)
                    _rebuild_task_chips(_ps, _pidx, panels)
                    _rebuild_hours_section(_ps, _pidx, panels)
                    _rebuild_preview(_ps, _pidx, panels)
                ...
                sections.append(ft.Column([
                    ft.Row([
                        ...
                        ft.TextButton("All", on_click=_sel_group,
                            style=ft.ButtonStyle(padding=ft.padding.all(_s(2)))),
                        ft.TextButton("None", on_click=_clr_group,
                            style=ft.ButtonStyle(
            ...
            if len(sections) <= 1:
                sections.append(ft.Text("No people match the search.",
        ...
        def _rebuild_task_chips(ps, pidx, all_panels):
            ...
            if not visible_tasks:
                ...
                for t in visible_tasks:
                    # blocked only if ALL selected persons lack the skill
                    no_skill = (persons
                                and all(not _has_skill(p, t) for p in persons))
                    is_sel   = t == ps["task"][0]
                    if no_skill:
        ...
        def _rebuild_hours_section(ps, pidx, all_panels):
            ...
            for d in days:
                ...
                for h in hours_map[d]:
                    # Unavailable only if ALL selected persons are unavailable
                    unavail   = (persons
                                 and all(not _is_avail(p, h, d) for p in persons))
                ...
                def _sel_all_h(ev, _d=d, _ps=ps, _pidx=pidx):
                    ...
                    _persons = _ps["persons"][0]
                    for _h in hours_map[_d]:
                        # Available if ANY selected person is available (or none selected)
                        avail_ok  = (not _persons
                                     or any(_is_avail(p, _h, _d) for p in _persons))
        ...
        def _build_panel_widget(ps, pidx, removable=False):
            ...
            return ft.Container(
                content=ft.Column([
                    header, preview_bar,
                    ft.Row([
                        _col_section(ft.Icons.PERSON, "Person(s)", "#6A1B9A",
        ...
        def _on_save(ev):
            ...
            if is_edit:
                s.mandatory_rules[edit_index:edit_index+1] = new_rules
            else:
        ...
        content = ft.Column([
            ft.Text(
                "Force one or more persons to work on a specific task, any available task, "
                "or force them to rest at chosen hours. "
                "Select multiple people to create the same rule for all of them. "
                "Tasks in red = no selected person has the skill. "
                "Hours in red = all selected persons unavailable or task has no demand.",
