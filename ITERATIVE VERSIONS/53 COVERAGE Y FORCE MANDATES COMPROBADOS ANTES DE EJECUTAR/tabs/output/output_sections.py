...
def build_coverage_gaps_section(messages: list) -> list:
    if not messages:
        return []
    controls = [section_header("COBERTURA INSUFICIENTE — El solver no puede ejecutarse", count=len(messages))]
    controls.append(ft.Container(
        content=ft.Text(
            "El problema no tiene solución con la configuración actual. "
            "Hay turnos donde no hay suficiente personal cualificado y disponible "
            "para cubrir la demanda. Debes corregir los siguientes conflictos antes de volver a intentarlo:",
            size=_s_stats(12), color="#BF360C", italic=True,
        ),
        margin=ft.margin.only(left=_s_stats(12), top=_s_stats(4), bottom=_s_stats(4)),
    ))
    controls.extend(violation_chip(m) for m in messages)
    return controls
