...
def _diff_icon(task, ref_task):
    ...
    return ft.Container(
        content=ft.Text(char, size=_s_grid(8), color=ft.Colors.WHITE,
                        weight=ft.FontWeight.BOLD),
        width=_s_grid(13), height=_s_grid(13), bgcolor=color, border_radius=7,
        alignment=ft.alignment.center, right=1, top=1)


def _header_cell(txt, w, h):
    return ft.Container(
        ft.Text(txt, size=_s_grid(11), weight=ft.FontWeight.BOLD,
    ...
    for t_idx, t in enumerate(tasks):
        ...
        max_names = max(
            (len(set(assigned_per_hour.get(h, [])) | set(ref_per_hour.get(h, [])))
             for h in hours_list),
            default=0)
        row_h = max(Ch, _s_grid(4) + _s_grid(18) * max(max_names, 1))

        name_cell = ft.Container(
            ft.Text(t, size=_s_grid(12), weight=ft.FontWeight.BOLD, color=t_fg,
                    no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS),
            width=NW, height=row_h, bgcolor=t_bg,
            alignment=ft.alignment.top_left,
            padding=ft.padding.only(left=_s_grid(8), top=_s_grid(4)),
        ...
        for h in hours_list:
            ...
            if assigned or (diff_mode == "cmp" and ref_per_hour.get(h)):
                name_rows = _build_diff_name_rows(
                    ...
                    ref_asgn_day, t_fg)
                inner = ft.Column(name_rows, spacing=_s_grid(1), tight=True)
                inner_cell = ft.Container(
                    inner, width=CW - _s_grid(4), height=row_h - _s_grid(4),
                    bgcolor=t_bg, alignment=ft.alignment.top_left,
                    padding=ft.padding.only(left=_s_grid(8), top=_s_grid(6)),
                    border_radius=6,
        ...
        cells.append(ft.Container(
            ft.Text(str(int(total)), size=_s_grid(12), weight=ft.FontWeight.BOLD,
...
def build_person_grid(day, hours_list, people, asgn_day, person_colors,
    ...
    for idx_p, p in enumerate(people):
        ...
        cells = [ft.Container(
            ft.Text(p, size=_s_grid(12), weight=ft.FontWeight.BOLD, color=p_color),
            width=NW, height=Ch, bgcolor=row_bg,
            alignment=ft.alignment.center_left,
            padding=ft.padding.only(left=_s_grid(8)),
            border=ft.border.all(1, "#CFD8DC"))]
        total = 0
            ...
            if task:
                ...
                cell = ft.Container(
                    ft.Text(task if task != TRAVEL_LABEL else "🚗",
                            size=_s_grid(11), weight=ft.FontWeight.BOLD,
...
def _diff_marker_row(char, bg_color, name, name_color, name_weight=None,
    ...
    return ft.Row([
        ft.Container(
            ft.Text(char, size=_s_grid(7), color=ft.Colors.WHITE,
                    weight=ft.FontWeight.BOLD),
            width=_s_grid(12), height=_s_grid(12), bgcolor=bg_color,
            border_radius=6, alignment=ft.alignment.center),
        ft.Text(name, size=_s_grid(10), color=name_color,
...
def _plain_name(p, t_fg):
    return ft.Text(p, size=_s_grid(10), color=t_fg,
                   no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS)
