...
from tabs.finder        import FinderTab
...
class StaffSchedulerApp:
    ...
    def _update_solve_blocked(self):
        s = self.state
        # Only the new group-equity validation gates the Solve button.
        # Other validation_errors keys are left untouched to preserve
        # existing behaviour.
        has_equity_err = bool(s.validation_errors.get("group_equity"))
        s.solve_blocked = has_equity_err
        if self._solve_btn is None:
            return
        blocked = s.solver_running or has_equity_err
        self._solve_btn.bgcolor  = ft.Colors.GREY_500 if blocked else PRIMARY_BLUE
        self._solve_btn.disabled = blocked
        self._solve_btn.tooltip  = (
            "Group equity is misconfigured — open 'Group equity settings' "
            "in the Dimensions tab to fix."
            if has_equity_err and not s.solver_running else None)
        try:
    ...
    def _apply_loaded_profile(self):
        ...
        # Reconcile group equity against the freshly loaded groups and
        # refresh validation state so the Solve button reflects reality.
    ...
    def _build_ui(self):
        ...
        self._finder_tab = FinderTab(s, self.page)
        ...
        self._page_contents = {
            ...
            10: self._finder_tab.get_container(),
        }
        self._builders = {
        ...
        _menu_def = [
            ...
            ("Finder",        ft.Icons.PERSON_SEARCH,  10),
        ]
