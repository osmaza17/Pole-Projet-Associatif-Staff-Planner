...
from solve_model_pace_16 import solve_model
...
class SolverController:
    ...
    def do_solve(self, e):
        ...
        # ── Reconcile & validate group equity BEFORE anything else ──
        s.reconcile_group_equity()
        eq_errors = s.validate_group_equity()
        if eq_errors:
            self._out._ct.controls = [
                ft.Text(
                    "Cannot solve — group equity configuration is invalid:",
                    color=ft.Colors.RED_400, size=_s(14),
                    weight=ft.FontWeight.BOLD),
                *[ft.Text(f"• {m}", color=ft.Colors.RED_400, size=_s(12))
                  for m in eq_errors],
                ft.Text(
                    "Open 'Group equity settings' from the Dimensions tab "
                    "to fix this.",
                    color=ft.Colors.GREY_600, size=_s(11), italic=True),
            ]
            self._switch(9)
            self._upd()
            return
        ...
        # ── Build numeric shares dict for the solver ─────────────────
        group_shares_num: dict = {}
        if s.eq_group_mode == "shares":
            for g_name, v in s.group_shares_st.items():
                raw = (v or "").strip()
                if not raw:
                    continue
                try:
                    group_shares_num[g_name] = float(raw)
                except ValueError:
                    pass
        ...
        data = dict(
            ...
            groups_work_ratios  = group_shares_num,
        )
