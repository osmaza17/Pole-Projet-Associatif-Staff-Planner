...
class AppState:
    ...
    def _init_defaults(self):
        ...
        # eq_group_mode controls the GLOBAL inter-group distribution.
        #   "off"    → all people in a single solver pool (default)
        #   "shares" → user-defined percentages or ratios per group
        self.eq_group_mode : str = "off"

        # {group_name: str}  — text values as entered in the UI, so empty
        # strings survive round-tripping. Normalization to floats happens
        # at solve time. Used only when eq_group_mode == "shares".
        self.group_shares_st: dict = {}
    ...
    # ── Group equity reconciliation / validation ──────────────────

    def reconcile_group_equity(self) -> list:
        """Sync group_shares_st with the current set of groups.
        Returns the ordered list of group names.

        - Stale share entries (groups no longer present) are purged.
        - Missing share entries are added as "" (empty, user must fill).
        """
        people = _parse_lines(self.people_text)
        groups = self.build_groups(people)
        ...
        # Shares: purge stale, add missing
        for g in list(self.group_shares_st.keys()):
            if g not in group_set:
                del self.group_shares_st[g]
        for g in group_names:
            self.group_shares_st.setdefault(g, "")

        return group_names

    def validate_group_equity(self) -> list:
        """(Re)compute validation errors for group equity config and
        return a list of human-readable error messages. Also updates
        self.validation_errors['group_equity'] accordingly.

        Only the 'shares' mode can produce blocking errors; 'off' is
        always valid."""
        errs = self.validation_errors.setdefault("group_equity", set())
        errs.clear()
        messages: list = []

        if getattr(self, "eq_group_mode", "off") != "shares":
            return messages

        group_names = self.reconcile_group_equity()
        if not group_names:
            return messages
        ...
        parsed: dict = {}
        for g in group_names:
            raw = (self.group_shares_st.get(g) or "").strip()
            if not raw:
                missing.append(g)
                continue
            try:
        ...
        if missing:
            errs.add("shares_incomplete")
            messages.append(
                "Custom shares: every group needs a non-negative number. "
                f"Missing or invalid: {', '.join(missing)}.")
            return messages

        if sum(parsed.values()) <= 1e-9:
            errs.add("shares_all_zero")
            messages.append(
                "Custom shares: at least one group must have a "
