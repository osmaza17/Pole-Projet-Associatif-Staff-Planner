...
def _check_coverage_feasibility(days, hours, demand_set_per_day, demand,
    ...
    for d in days:
        ...
        for h in hours[d]:
            ...
            if not tasks_demand_h:
                continue
            task_people_h = {
                t: [p for p in people_per_task[t] if (p, h, d) in available_today]
                for t, _ in tasks_demand_h
            }
            for t, req, avail, deficit in _match(tasks_demand_h, task_people_h):
                gaps.append({
                    "day": d, "hour": h, "task": t,
                    "required": req, "available": avail, "deficit": deficit,
                })
    return gaps
...
def solve_model(data, ui_update_callback=None, active_model_ref=None):
    ...
    coverage_gaps = _check_coverage_feasibility(days, hours, demand_set_per_day, demand, people_per_task, available_set_per_day)
    if coverage_gaps:
        messages = [
            f"❌  {g['day']} a las {g['hour']}  —  Tarea '{g['task']}':  "
            f"se necesitan {g['required']} persona(s) pero solo hay {g['available']} "
            f"cualificada(s) Y disponible(s) en ese momento  "
            f"(faltan {g['deficit']}).  "
            f"Para resolverlo: marca a {g['deficit']} persona(s) más como disponibles "
            f"en ese horario, o cualifícalas para '{g['task']}', "
            f"o reduce la demanda de '{g['task']}' a {g['available']} en ese turno."
            for g in coverage_gaps
        ]
        return {
            "status":                 "Infeasible: coverage gaps detected",
    ...
    sol["coverage_gaps"] = []  # vacío: si llegamos aquí, la cobertura es factible
