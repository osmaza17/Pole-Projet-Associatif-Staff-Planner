...
class CaptainRulesManager:
    ...
    def __init__(self, state, page: ft.Page, captain_rules_col: ft.ListView):
        ...
        self._rule_errors : dict = {}   # {idx: [error_str, …]}
    ...
    def validate_and_refresh(self):
        """
        Check every saved captain rule. Error conditions:

        1. Captain(s) no longer in any group.
        2. No demand for the task at a given (h, d) slot.
        3. No captain is both available AND skilled for the task at a
           given (h, d) slot — covers three sub-cases:
             a. ALL captains globally lack skill for the task.
             b. All captains are unavailable at the slot (regardless of skill).
             c. All captains that are available at the slot lack skill.
           Sub-case (a) is reported at rule level; (b)/(c) are reported
           per slot with a unified "no available+skilled captain" message.
        """
        s = self._state
        people, _, _, _ = s.dims()
        ...
        for idx, rule in enumerate(s.captain_rules):
            ...
            # Only check the captains that still exist
            ...
            if active_caps and t:
                # ── 4. Demand still positive for each (t, h, d)? ───────
                no_demand = []
                ...
                # ── 3a. Does ANY captain have skill for the task? ───────
                skilled_caps = [
                ...
                if not skilled_caps:
                    ...
                    # ── 3b/c. Per-slot: available AND skilled? ──────────
                    # A slot is only satisfiable if at least one captain
                    # is both available (avail=1 or 2) AND has skill.
                    bad_slots = []
                    ...
                    if bad_slots:
                        ...
                        errs.append(
                            f"No available+skilled captain for '{t}' at: "
                            f"{', '.join(shown)}{tail}")

            elif active_caps and not t:
                # No task defined — only check availability per slot
                bad_slots = []
                for d, hs in hrs.items():
                    for h in hs:
                        avail_caps = [
        ...
        # Replace old captain_* keys with fresh ones
    ...
    def open_dialog(self, e, edit_index=None):
        ...
        def _panel_can_save(ps):
            ...
            return bool(ps["sel_captains"]) and ps["sel_task"][0] and any_hour
        ...
        def _build_panel_widget(ps, pidx, removable=False):
            ...
            def _sel_all_caps(ev, _ps=ps):
                ...
                _rebuild_captain_chips(_ps); _rebuild_preview(_ps); _refresh_save_btn()
        ...
        content = ft.Column([
            ft.Text(
                "Define which people (captains) must be present (≥ 1) for "
