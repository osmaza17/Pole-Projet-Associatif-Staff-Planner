...
class MaxConsecHoursManager:
    ...
    def open_dialog(self, e):
        ...
        # ── Bulk Personalize / Reset All button ───────────────────────
        ...
        def _refresh_bulk_btn():
            """Update the bulk button label/color to reflect current state."""
            all_personalized = bool(people) and all(
                p in s.consec_personalized_persons for p in people)
            if all_personalized:
                icon  = ft.Icons.RESTART_ALT
                label = "Reset All"
                color = ft.Colors.RED_700
                bdr   = ft.border.all(1, ft.Colors.RED_700)
            else:
                icon  = ft.Icons.EDIT
                label = "Personalize All"
                color = "#1565C0"
                bdr   = ft.border.all(1, "#1565C0")
        ...
        def _on_bulk_click(ev):
            all_personalized = bool(people) and all(
                p in s.consec_personalized_persons for p in people)
            if all_personalized:
                # Reset ALL → remove everyone, sync back to global
                for p in people:
                    s.consec_personalized_persons.discard(p)
                    s.consec_per_person[p]      = s.consec_global_val
                    s.consec_rest_per_person[p] = s.consec_global_rest
                    s.validation_errors["consec"].discard(p)
                    s.validation_errors["consec"].discard(f"{p}_rest")
                self._on_solve_blocked_update()
            else:
                # Personalize ALL → add everyone (keep their current values)
                for p in people:
