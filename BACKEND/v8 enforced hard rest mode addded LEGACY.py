...
def solve_model(data):
    ...
    if consec_keys: obj += [W_ROTATION * c[k] for k in consec_keys]
...
def main(page: ft.Page):
    ...
    def build_avail():
        ...
        for j in days:
            ...
            for p in people:
                avail_ct.controls.append(ft.Row([lbl(p)] + [make_toggle(avail_st,(p,h,j), 1) for h in hours], spacing=2))
    ...
    def build_skills():
        ...
        for p in people:
            skills_ct.controls.append(ft.Row([lbl(p)] + [make_toggle(skills_st,(p,t), 1) for t in tasks], spacing=2))
        page.update()
    ...
    def build_force():
        ...
        for t in tasks:
            for j in days:
                ...
                for p in people:
                    force_ct.controls.append(ft.Row([lbl(p)] + [make_toggle(force_st,(p,t,h,j), 0) for h in hours], spacing=2))
    ...
    def build_social():
        ...
        for i, p1 in enumerate(people):
            ...
            if cells: social_ct.controls.append(ft.Row([lbl(p1)] + cells, spacing=2))
        page.update()
    ...
    def build_output_grid(sol, people,tasks,hours,days, availability):
        ...
        for j in days:
            ...
            for idx_p,p in enumerate(people):
                ...
                cells = [ft.Container(ft.Text(p,size=12, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE), width=NW, height=Ch,bgcolor=row_bg, alignment=ft.alignment.center_left, padding=ft.padding.only(left=8), border=ft.border.all(1, "#455A64"))]
                total = 0
                for h in hours:
    ...
    dim_tab_content = ft.Container(content=ft.Row(controls=[tf_people, tf_tasks, tf_hours, tf_days, enforced_rest_col], spacing=20, alignment=ft.MainAxisAlignment.START, vertical_alignment=ft.CrossAxisAlignment.START), padding=20, expand=True)
