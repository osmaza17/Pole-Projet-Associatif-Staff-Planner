...
EMERG_COLOR   = "#F57C00"
AVAIL_COLOR   = "#388E3C"
DIFF_COLOR    = "#FF6D00"   # Tono de naranja distinto al de emergencia para el diff
...
def main(page: ft.Page):
    ...
    #
    # El diff_mode y ref_sol se determinan internamente desde diff_state,
    # por lo que no se pasan como parámetros externos.
            ...
            for j in days:
                ...
                for idx_p, p in enumerate(people):
                    ...
                    for h in day_hours:
                        ...
                        # ── Lógica de borde según modo ────────────────
                        # En modo diff (ref o cmp) se suprimen los bordes
                        # de disponibilidad en las dos columnas comparadas.
                        # El borde naranja de diff (DIFF_COLOR) aparece
                        # únicamente en la columna "Comparado".
                        if diff_mode in ("ref", "cmp"):
                            if diff_mode == "cmp":
                                ref_task = ref_assignment.get(j, {}).get(p, {}).get(h)
                                if task != ref_task:
                                    brd = ft.border.all(2.5, DIFF_COLOR)
                                else:
                                    brd = ft.border.all(0.5, "#CFD8DC")
                            else:
                                # Columna de referencia: borde neutro
                                brd = ft.border.all(0.5, "#CFD8DC")
                        else:
                            # Fuera de modo diff: bordes de disponibilidad normales
                ...
                legend_items = [
                    ft.Container(
                        ...
                        border_radius=4)
                    for t in tasks]
                # En modo diff se muestra la leyenda del borde de diff en lugar
                # de los bordes de disponibilidad
                if diff_mode in ("ref", "cmp"):
                    if diff_mode == "cmp":
                        legend_items.append(ft.Container(
                            ft.Text("Diferencia vs Referencia", size=10, color=ft.Colors.WHITE),
                            bgcolor=DIFF_COLOR, padding=ft.padding.symmetric(6, 10),
                            border_radius=4))
                else:
