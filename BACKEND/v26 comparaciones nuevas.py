...
from solve_model_pace_4 import solve_model
...
# Colores de los círculos de diff: verde (+), rojo (−), naranja (⇄)
...
def main(page: ft.Page):
    ...
    quota_filter_st  = [None]         # [day]  — quota is now per (p, t, d)
    ...
    # ── Diff state ───────────────────────────────────────────
    # ref: índice del run de referencia; cmp: índice del run comparado.
    # None significa que ese slot está vacío.
    ...
    def build_quota():
        # Quota is now per (p, t, d): one matrix per day, identical navigation
        # to build_demand. No _needs_rebuild guard — day selection always triggers
        # a full rebuild of this tab.
        people, tasks, hours, days = dims()
        if not days:
            return
    ...
    #
    # Gestiona la selección de dos runs para comparar.
    # Reglas:
    #   · Primer click → "Referencia" (azul claro)
    #   · Segundo click → "Comparado" (ámbar)
    #   · Click sobre "Referencia" activo → se deselecciona; si había
    #     "Comparado", pasa automáticamente a ser "Referencia"
    #   · Click sobre "Comparado" activo → se deselecciona
    #   · Click sobre un botón no seleccionado cuando los dos slots
    #     están ocupados → se ignora
    def _handle_diff_click(run_idx):
        if run_idx == diff_state["ref"]:
            # Deseleccionar referencia; el comparado (si existe) asciende a referencia
            diff_state["ref"] = diff_state["cmp"]
            diff_state["cmp"] = None
        elif run_idx == diff_state["cmp"]:
            # Deseleccionar comparado
            diff_state["cmp"] = None
        elif diff_state["ref"] is None:
            # Primer slot libre → este run pasa a ser referencia
            diff_state["ref"] = run_idx
        elif diff_state["cmp"] is None:
            # Segundo slot libre → este run pasa a ser comparado
            diff_state["cmp"] = run_idx
        # else: ambos slots ocupados y se ha pulsado un botón no seleccionado → ignorar
        _rebuild_output()
        page.update()

    # ── Diff icon helper ──────────────────────────────────────
    #
    # Construye el círculo con el icono de diff que se superpone en la
    # esquina superior derecha de la celda usando ft.Stack.
    # Devuelve None si no hay cambio, o un ft.Container posicionado.
    def _diff_icon(task, ref_task):
        """Calcula el tipo de cambio y devuelve el overlay o None."""
        if task == ref_task:
            return None
        if ref_task is None:
            # vacío → asignado: nueva asignación
            char  = "+"
            color = DIFF_ADD_COLOR
        elif task is None:
            # asignado → vacío: asignación eliminada
            char  = "−"
            color = DIFF_REMOVE_COLOR
        else:
            # tarea A → tarea B: cambio de tarea
            char  = "⇄"
            color = DIFF_CHANGE_COLOR
        return ft.Container(
    ...
    def _build_run_column(run_idx, sol, people, tasks, hours, days,
        ...
        # ── Determinar modo diff desde diff_state (closure) ───────────
        # diff_mode: None | "ref" | "cmp"
        # ref_sol: solución del run de referencia (solo cuando diff_mode=="cmp")
        if not is_live:
            if run_idx == diff_state["ref"]:
                diff_mode = "ref"
            elif run_idx == diff_state["cmp"]:
        ...
        # Ambos slots ocupados → los botones "Comparar" sin selección se bloquean
        ...
        # ── Botón de diff ─────────────────────────────────────
        # Estados visuales:
        #   "Comparar" (libre)    → fondo verde claro, clickable
        #   "Referencia"          → fondo azul claro, clickable (para deseleccionar)
        #   "Comparado"           → fondo ámbar, clickable (para deseleccionar)
        #   "Comparar" (bloqueado)→ fondo gris, no clickable
        #   Live column           → fondo gris, no clickable
        if is_live:
            diff_lbl   = "Comparar"
            diff_bg    = ft.Colors.GREY_400
            diff_fg    = ft.Colors.GREY_600
            diff_click = None
        elif diff_mode == "ref":
            diff_lbl   = "Referencia"
            diff_bg    = ft.Colors.LIGHT_BLUE_300
            diff_fg    = ft.Colors.BLACK
            diff_click = lambda e, _i=run_idx: _handle_diff_click(_i)
        elif diff_mode == "cmp":
            diff_lbl   = "Comparado"
            diff_bg    = ft.Colors.AMBER_300
            diff_fg    = ft.Colors.BLACK
            diff_click = lambda e, _i=run_idx: _handle_diff_click(_i)
        elif both_selected:
            diff_lbl   = "Comparar"
        ...
        # En modo diff las celdas son más altas para que el icono overlay sea legible.
        CW = 40; NW = 50; TW = 25
        Ch = 26
        ...
        if is_live and is_solving and not any(assignment.get(d) for d in days):
            ...
            # Asignación del run de referencia para calcular el diff celda a celda
            ...
            for j in days:
                ...
                for idx_p, p in enumerate(people):
                    ...
                    for h in day_hours:
                        ...
                        # Bordes de disponibilidad: siempre presentes en todas las columnas
                        ...
                        # ── Icono de diff (solo en la columna "Comparado") ────
                        # Si hay cambio respecto a la referencia, se superpone un
                        # círculo con el símbolo del tipo de cambio en la esquina
                        # superior derecha usando ft.Stack.
                ...
                # En la columna "Comparado" se añaden los tres iconos de diff a la leyenda
                if diff_mode == "cmp":
            ...
            if not (is_live and is_solving):
                ...
                for p in people:
                    ...
                    if diff_mode == "cmp" and ref_sol is not None:
                        # Resaltar en naranja las personas con carga distinta;
                        # mostrar entre paréntesis el valor del run de referencia
    ...
    def _rebuild_output(live_sol=None, live_people=None, live_tasks=None,
        ...
        for i, entry in enumerate(solution_history):
            def _make_delete(idx):
                def _delete():
                    # Ajustar diff_state al borrar un run
                    if diff_state["ref"] == idx:
                        # La referencia desaparece; el comparado (si existe) asciende
                        diff_state["ref"] = diff_state["cmp"]
                        diff_state["cmp"] = None
                    elif diff_state["cmp"] == idx:
                        diff_state["cmp"] = None
                    # Decrementar índices de runs posteriores al borrado
                    for key in ("ref", "cmp"):
    ...
    def do_solve(e):
        ...
        # min_quota indexed by (p, t, d) — one value per person, task, and day
        mq = {}
        for p in people:
            for t in tasks:
                for j in days:
