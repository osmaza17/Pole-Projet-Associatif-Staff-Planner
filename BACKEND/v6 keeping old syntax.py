...
def solve_model(data):
    ...
    # c[(p,t,h,j)] - Sparse: only for tasks with rotation=1
    consec_keys = [
        (p,t,h,j)
    ...
    # r[(p,h,j)] - Shift Start / Rising Edge
    ...
    # 8. Gap / Shift Start Penalty (Using 'r')
    ...
    # --- E. Double (Daily and Global) Equity ---
    for j in days:
        for p in people:
    ...
    # --- J. Shift Continuity / Block Counting (With Tuple Keys) ---
    for j in days:
        for p in people:
            # Hour 0
...
# ============================================================
# FLET UI (Same as before, just ensuring imports)
# ============================================================
...
def main(page: ft.Page):
    ...
    tf_tasks = ft.TextField(
        value="Buy Milk\nMeditate\nPlan Week", multiline=True, min_lines=8, max_lines=200,
        ...
        for j in days:
            ...
            for p in people:
                avail_ct.controls.append(ft.Row(
                    [lbl(p)] + [make_toggle(avail_st,(p,h,j), 1) for h in hours], spacing=2))
    ...
    def build_skills():
        ...
        for p in people:
            skills_ct.controls.append(ft.Row(
                [lbl(p)] + [make_toggle(skills_st,(p,t), 1) for t in tasks], spacing=2))
        page.update()
            ...
            for j in days:
                ...
                for p in people:
                    force_ct.controls.append(ft.Row(
                        [lbl(p)] + [make_toggle(force_st,(p,t,h,j), 0) for h in hours], spacing=2))
