...
from solve_model_pace_1 import solve_model
...
def main(page: ft.Page):
    ...
    def build_output_grid(sol, people, tasks, hours, days, availability, emergency):
        ...
        for j in days:
            ...
            def _hdr_cell(txt):
                return ft.Container(
                    ft.Text(txt, size=11, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                    bgcolor="#546E7A", alignment=ft.alignment.center,
                    border=ft.border.all(1, "#455A64"))

            hdr = ([ft.Container(_hdr_cell("Person").content, width=NW, height=Ch,
                                 bgcolor="#546E7A", alignment=ft.alignment.center,
                                 border=ft.border.all(1, "#455A64"))] +
                   [ft.Container(ft.Text(h, size=11, weight=ft.FontWeight.BOLD,
                                 ...
                                 border=ft.border.all(1, "#455A64")) for h in day_hours] +
            ...
            legend_items = [
                ft.Container(ft.Text(t, size=10, weight=ft.FontWeight.BOLD, color=tc[t][1]),
                             ...
                             border_radius=4) for t in tasks]
    ...
    def do_solve(e):
        ...
        data = dict(
            ...
            day_heuristics=1 if day_heuristics_sw.value else 0,   # ← NUEVO
        )
        ...
        def run_solver():
            ...
            if output_ct.controls and isinstance(output_ct.controls[0], ft.ProgressRing):
