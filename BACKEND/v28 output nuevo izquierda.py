...
from solve_model_pace_5 import solve_model
...
def main(page: ft.Page):
    ...
    # ── Enforced rest state ───────────────────────────────────
    # consec_global_val_st: value of the global "max consec hours" textfield (str)
    # consec_personalized_st: whether per-person mode is active
    # consec_per_person_st: {person: str} individual max consec values
    consec_global_val_st   = [""]
    consec_personalized_st = [False]
    consec_per_person_st   = {}

    # Pagination / Dropdown States
    avail_filter_st  = [None]
    demand_filter_st = [None]
    force_filter_st  = [None, None]
    quota_filter_st  = [None]

    running_model_ref = [None]
    ui_lock = threading.Lock()
    validation_errors = {"demand": set(), "quota": set(), "consec": set()}
    solve_blocked = [False]
    solver_running = [False]

    solution_history = []
    diff_state = {"ref": None, "cmp": None}
    ...
    def _update_solve_blocked():
        has_errors = bool(
            validation_errors["demand"] or
            validation_errors["quota"]  or
            validation_errors["consec"])
        solve_blocked[0] = has_errors
        blocked = has_errors or solver_running[0]
        solve_btn_ref[0].bgcolor = ft.Colors.GREY_500 if blocked else "#1565C0"
        solve_btn_ref[0].disabled = blocked
        try:
    ...
    def _validate_positive_int(value):
        """Accepts empty string (no constraint) or integers >= 1."""
        v = value.strip()
        if v == "":
            return True
        try:
    ...
    # ── Enforced Rest controls ────────────────────────────────
    # err_consec must be defined before build_captains_list and enforced_rest_col
    err_consec = ft.Text("", color=ft.Colors.RED_400, size=11, visible=False)

    def _update_err_consec():
        has_err = bool(validation_errors["consec"])
        err_consec.value   = "⚠ Max consec: only positive integers (≥ 1) allowed." if has_err else ""
        err_consec.visible = has_err
        try:
            err_consec.update()
        except Exception:
            pass

    tf_consec_global = ft.TextField(
        value="", width=110, height=38, text_size=12,
        hint_text="Max consec", content_padding=ft.padding.all(4))

    def _on_consec_global_change(e):
        val = e.control.value
        consec_global_val_st[0] = val
        # Always validate the global field
        if _validate_positive_int(val):
            validation_errors["consec"].discard("_global")
            e.control.border_color = None
        else:
            validation_errors["consec"].add("_global")
            e.control.border_color = ft.Colors.RED_400
        # If not personalized, sync all per-person values and rebuild
        if not consec_personalized_st[0]:
            people_now = list(dict.fromkeys(
                p.strip() for p in tf_people.value.split("\n") if p.strip()))
            for p in people_now:
                consec_per_person_st[p] = val
            build_captains_list()
        _update_err_consec()
        _update_solve_blocked()
        e.control.update()

    tf_consec_global.on_change = _on_consec_global_change

    def _on_personalize_toggle(e):
        is_on = e.control.value
        consec_personalized_st[0] = is_on
        people_now = list(dict.fromkeys(
            p.strip() for p in tf_people.value.split("\n") if p.strip()))
        if not is_on:
            # Turning OFF personalization: sync all individual fields to global value
            global_val = consec_global_val_st[0]
            for p in people_now:
                consec_per_person_st[p] = global_val
            # Clear per-person validation errors (global field already validated separately)
            for key in list(validation_errors["consec"]):
                if key != "_global":
                    validation_errors["consec"].discard(key)
            _update_err_consec()
            _update_solve_blocked()
        # Turning ON: individual fields keep their current values (= global if was OFF)
        build_captains_list()

    sw_consec_personalize = ft.Switch(
        label="Personalize Rest", value=False,
        on_change=_on_personalize_toggle)

    enforced_rest_col = ft.Column([
        ft.Row([sw_consec_personalize, tf_consec_global],
    ...
    def build_captains_list(e=None):
        people = list(dict.fromkeys(p.strip() for p in tf_people.value.split("\n") if p.strip()))

        # Clean up state for removed people
        for p_old in list(consec_per_person_st.keys()):
            if p_old not in people:
                consec_per_person_st.pop(p_old, None)
                validation_errors["consec"].discard(f"person_{p_old}")
        _update_err_consec()
        _update_solve_blocked()

        is_personalized = consec_personalized_st[0]
        buf = [ft.Text("Cap / Rest", weight=ft.FontWeight.BOLD, size=12)]

        for p in people:
            # Initialize captain state
            if p not in captains_st:
                captains_st[p] = 0
            # Initialize per-person consec state
            if p not in consec_per_person_st:
                consec_per_person_st[p] = consec_global_val_st[0]
            ...
            # Per-person max consecutive hours textfield
            consec_val = consec_per_person_st.get(p, "")
            is_valid   = _validate_positive_int(consec_val)
            tf_p = ft.TextField(
                value=consec_val,
                width=52, height=28, text_size=11,
                content_padding=ft.padding.all(2),
                disabled=not is_personalized,
                border_color=ft.Colors.RED_400 if (not is_valid and is_personalized) else None)

            def _ch_consec(e, _p=p):
                consec_per_person_st[_p] = e.control.value
                if _validate_positive_int(e.control.value):
                    validation_errors["consec"].discard(f"person_{_p}")
                    e.control.border_color = None
                else:
                    validation_errors["consec"].add(f"person_{_p}")
                    e.control.border_color = ft.Colors.RED_400
                _update_err_consec()
                _update_solve_blocked()
                e.control.update()
            tf_p.on_change = _ch_consec

            buf.append(ft.Row([ft.Text(p, size=11, width=85), cap_btn, tf_p], spacing=4))
    ...
    param_tf_list = list(param_tfs.values())
    param_col1    = ft.Column(param_tf_list[::2],  spacing=8)
    param_col2    = ft.Column(param_tf_list[1::2], spacing=8)
    params_section = ft.Column([
        ...
        ft.Row([param_col1, param_col2], spacing=8,
               vertical_alignment=ft.CrossAxisAlignment.START),
    ], spacing=10)

    right_controls_col = ft.Column(
        [enforced_rest_col, ft.Divider(), params_section], width=300)
    ...
    def build_avail():
        ...
        nav_day = make_nav_dropdown("Select Day", avail_filter_st[0], days, _on_day_change, 200)
        buf = [ft.Row([
            nav_day,
        ...
        buf.append(ft.Row(
            ...
            [ft.Container(ft.Text(l, size=9, no_wrap=True, overflow=ft.TextOverflow.CLIP),
    ...
    def build_demand():
        ...
        nav_day = make_nav_dropdown("Select Day", demand_filter_st[0], days, _on_day_change, 200)
    ...
    def build_quota():
        ...
        if not days:
            return
        if quota_filter_st[0] not in days:
            quota_filter_st[0] = days[0]

        def _on_day_change(new_val):
            quota_filter_st[0] = new_val
            build_quota()
        ...
        def _rand_quota(e):
            for p in people:
                for t in tasks:
                    for j in days:
                        quota_st[(p, t, j)] = str(random.randint(0, 2))
            validation_errors["quota"].clear()
            _update_solve_blocked()
            build_quota()
        ...
        nav_day = make_nav_dropdown("Select Day", quota_filter_st[0], days, _on_day_change, 200)
        ...
        buf = [
            ...
            ft.Row([
                nav_day,
                make_reset_btn("Reset All Days", _reset_quota),
                ft.Container(
                    content=ft.Text("Set All to 0", color=ft.Colors.WHITE,
                                    weight=ft.FontWeight.BOLD, size=12),
                    bgcolor=ft.Colors.ORANGE_700, padding=10, border_radius=4,
                    on_click=_zero_quota, width=150, alignment=ft.alignment.center),
                ft.Container(
                    content=ft.Text("Random Quota (All Days)", color=ft.Colors.WHITE,
                                    weight=ft.FontWeight.BOLD, size=12),
                    bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4,
                    on_click=_rand_quota, width=180, alignment=ft.alignment.center),
            ], spacing=20),
            quota_error_text,
        ]
    ...
    def build_force():
        ...
        nav_day  = make_nav_dropdown("Select Day",  force_filter_st[0], days,  _on_day_change,  150)
        nav_task = make_nav_dropdown("Select Task", force_filter_st[1], tasks, _on_task_change, 200)
    ...
    # ── Diff helpers ─────────────────────────────────────────
    def _handle_diff_click(run_idx):
        if run_idx == diff_state["ref"]:
            diff_state["ref"] = diff_state["cmp"]
            diff_state["cmp"] = None
        elif run_idx == diff_state["cmp"]:
            diff_state["cmp"] = None
        elif diff_state["ref"] is None:
            diff_state["ref"] = run_idx
        elif diff_state["cmp"] is None:
            diff_state["cmp"] = run_idx
        _rebuild_output()
        page.update()
    ...
    def _diff_icon(task, ref_task):
        ...
        return ft.Container(
            content=ft.Text(char, size=8, color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD),
    ...
    def _build_run_column(run_idx, sol, people, tasks, hours, days,
        ...
        ref_sol = None
        if diff_mode == "cmp" and diff_state["ref"] is not None:
            ref_idx = diff_state["ref"]
            if 0 <= ref_idx < len(solution_history):
                ref_sol = solution_history[ref_idx]["sol"]

        both_selected = diff_state["ref"] is not None and diff_state["cmp"] is not None
        buf = []

        if is_live:
            diff_lbl, diff_bg, diff_fg, diff_click = "Comparar", ft.Colors.GREY_400, ft.Colors.GREY_600, None
        elif diff_mode == "ref":
            diff_lbl, diff_bg, diff_fg = "Referencia", ft.Colors.LIGHT_BLUE_300, ft.Colors.BLACK
            diff_click = lambda e, _i=run_idx: _handle_diff_click(_i)
        elif diff_mode == "cmp":
            diff_lbl, diff_bg, diff_fg = "Comparado", ft.Colors.AMBER_300, ft.Colors.BLACK
            diff_click = lambda e, _i=run_idx: _handle_diff_click(_i)
        elif both_selected:
            diff_lbl, diff_bg, diff_fg, diff_click = "Comparar", ft.Colors.GREY_400, ft.Colors.GREY_600, None
        else:
            diff_lbl, diff_bg, diff_fg = "Comparar", ft.Colors.GREEN_200, ft.Colors.BLACK
            diff_click = lambda e, _i=run_idx: _handle_diff_click(_i)

        diff_btn = ft.Container(
            content=ft.Text(diff_lbl, size=11, color=diff_fg,
                            weight=ft.FontWeight.BOLD, no_wrap=True),
            bgcolor=diff_bg, padding=ft.padding.symmetric(4, 8),
            border_radius=4, on_click=diff_click)

        if is_live and is_solving:
            hdr_label, hdr_bg = f"⟳  Run #{run_idx + 1}  ·  Solving…", "#E65100"
        else:
            hdr_label, hdr_bg = f"Run #{run_idx + 1}  ·  {status}", "#1565C0"

        hdr_content = ft.Row(
            controls=[
                ft.Text(hdr_label, weight=ft.FontWeight.BOLD, size=13,
        ...
        buf.append(ft.Container(
            content=hdr_content, bgcolor=hdr_bg,
        ...
        if not (is_live and is_solving):
            ...
            buf.append(ft.Text(f"⏱  Solve time: {solve_time:.1f} s",
                               size=12, italic=True, color=ft.Colors.GREY_700))
            buf.append(ft.Text(f"◎  MIP Gap: {mip_gap * 100:.3f} %",
                               size=12, italic=True, color=ft.Colors.GREY_700))
        ...
        if is_live and is_solving and not any(assignment.get(d) for d in days):
            ...
            ref_assignment = ref_sol.get("assignment", {}) if ref_sol else {}
            ...
            for j in days:
                ...
                hdr = (
                    [ft.Container(ft.Text("Person", size=11, weight=ft.FontWeight.BOLD,
                                          color=ft.Colors.WHITE),
                                  width=NW, height=Ch, bgcolor="#546E7A",
                                  alignment=ft.alignment.center,
                                  border=ft.border.all(1, "#455A64"))] +
                    [ft.Container(ft.Text(h, size=11, weight=ft.FontWeight.BOLD,
                                          color=ft.Colors.WHITE),
                                  width=CW, height=Ch, bgcolor="#546E7A",
                                  alignment=ft.alignment.center,
                                  border=ft.border.all(1, "#455A64"))
                     for h in day_hours] +
                    [ft.Container(ft.Text("Total", size=11, weight=ft.FontWeight.BOLD,
                                          color=ft.Colors.WHITE),
                ...
                for idx_p, p in enumerate(people):
                    ...
                    for h in day_hours:
                        ...
                        if diff_mode == "cmp" and ref_sol is not None:
                            ref_task = ref_assignment.get(j, {}).get(p, {}).get(h)
            ...
            if not (is_live and is_solving):
                ...
                for p in people:
                    curr_h = workload.get(p, 0)
                    if diff_mode == "cmp" and ref_sol is not None:
                        prev_h = ref_workload.get(p, 0)
                        if curr_h != prev_h:
                            buf.append(ft.Text(
                                f"  {p}: {curr_h:.0f} h  ({prev_h:.0f} prev)",
                                size=12, color=ft.Colors.ORANGE_700, weight=ft.FontWeight.BOLD))
                        else:
                            buf.append(ft.Text(f"  {p}: {curr_h:.0f} hours", size=12))
                    else:
                        buf.append(ft.Text(f"  {p}: {curr_h:.0f} hours", size=12))
                buf.append(ft.Text(
                    f"  Global range: max={sol.get('workload_max', 0):.0f}, min={sol.get('workload_min', 0):.0f}",
                    size=12, italic=True))
    ...
    def _rebuild_output(live_sol=None, live_people=None, live_tasks=None,
        ...
        for i, entry in enumerate(solution_history):
            def _make_delete(idx):
                def _delete():
                    if diff_state["ref"] == idx:
                        diff_state["ref"] = diff_state["cmp"]
                        diff_state["cmp"] = None
                    elif diff_state["cmp"] == idx:
                        diff_state["cmp"] = None
                    for key in ("ref", "cmp"):
                        if diff_state[key] is not None and diff_state[key] > idx:
                            diff_state[key] -= 1
                    solution_history.pop(idx)
                    _rebuild_output()
                    page.update()
                return _delete
        ...
        cols.reverse()  # newest run on the left

        if live_sol is not None:
            cols.insert(0, _build_run_column(
    ...
    def do_solve(e):
        ...
        # Build per-person max consecutive hours dict
        # Empty dict → no enforced-rest constraint for anyone
        max_consec_hours = {}
        if consec_personalized_st[0]:
            for p in people:
                raw = consec_per_person_st.get(p, "").strip()
                if raw:
                    try:
                        max_consec_hours[p] = int(raw)
                    except ValueError:
                        pass
        else:
            raw = consec_global_val_st[0].strip()
            if raw:
                try:
                    n = int(raw)
                    for p in people:
                        max_consec_hours[p] = n
                except ValueError:
                    pass
        ...
        data = dict(
            ...
            max_consec_hours=max_consec_hours,
        ...
        def run_solver():
            try:
                ...
                solver_running[0] = False
                _update_solve_blocked()
            page.update()

        solver_running[0] = True
        _update_solve_blocked()
    ...
    dim_tab_content = ft.Container(
        content=ft.Row(
            controls=[right_controls_col, tf_people, captains_col, tf_tasks,
                      rotation_col, tf_days, hours_col],
    ...
    for label, icon, idx in menu_items_def:
        ...
        btn = ft.Container(
            ...
            on_click=_on_menu_click, data=idx, ink=True)
    ...
    sidebar = ft.Container(
        ...
        content=ft.Column(
            controls=[
                ...
                ft.Container(
                    content=ft.Column([solve_btn, day_heuristics_sw, stop_btn], spacing=6),
                    padding=ft.padding.only(bottom=8)),
                ft.Divider(color="#455A64", height=1),
                ft.Column(controls=menu_btn_refs, spacing=2,
