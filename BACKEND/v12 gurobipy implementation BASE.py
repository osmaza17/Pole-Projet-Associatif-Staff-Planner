...
def solve_model(data, ui_update_callback=None):
    ...
    z_max = model.addVar(lb=0, vtype=GRB.INTEGER, name="z_max")
    z_min = model.addVar(lb=0, vtype=GRB.INTEGER, name="z_min")

    consec_keys = [(p,t,h,j) for p in people for t in tasks for h in hours[:-1] for j in days if rotation.get(t, 0) == 1]
    c = model.addVars(consec_keys, vtype=GRB.BINARY, name="c")

    friend_pairs = [(p1,p2) for (p1,p2), val in social.items() if val == 1]
    enemy_pairs  = [(p1,p2) for (p1,p2), val in social.items() if val == -1]
    v = model.addVars(friend_pairs, tasks, hours, days, vtype=GRB.BINARY, name="v")
    w = model.addVars(enemy_pairs, tasks, hours, days, vtype=GRB.BINARY, name="w")

    k = model.addVars(hours, days, vtype=GRB.BINARY, name="k")
    ...
    for p in people:
        for t in tasks:
            if skills.get((p,t), 0) == 0:
                model.addConstrs((x[p,t,h,j] == 0 for h in hours for j in days))
    ...
    for (p,t,h,j) in consec_keys:
        next_h = hours[hours.index(h) + 1]
        model.addConstr(x[p,t,h,j] + x[p,t,next_h,j] - c[p,t,h,j] <= 1)

    for (p1, p2) in friend_pairs:
        for t in tasks:
            for h in hours:
                for j in days:
                    model.addConstr(x[p1,t,h,j] - x[p2,t,h,j] <= v[p1,p2,t,h,j])
                    model.addConstr(x[p2,t,h,j] - x[p1,t,h,j] <= v[p1,p2,t,h,j])

    for (p1, p2) in enemy_pairs:
        for t in tasks:
            for h in hours:
                for j in days:
                    model.addConstr(x[p1,t,h,j] + x[p2,t,h,j] - w[p1,p2,t,h,j] <= 1)
        ...
        for t in tasks:
            for h in hours:
                for j in days:
                    ...
                    model.addConstr(d[p,t,h,j] >= prev - x[p,t,h,j])
                    model.addConstr(d[p,t,h,j] >= x[p,t,h,j] - prev)
    ...
    for t in tasks:
        for h in hours:
            for j in days:
                val = m[t,h,j].X
                if val > 0.01: missing.append(f"{t} @ {h}, {j}: {val:.0f} missing")
    sol["missing"] = missing

    workload = {}
    for p in people:
        total_hrs = sum(x[p,t,h,j].X for t in tasks for h in hours for j in days if x[p,t,h,j].X > 0.5)
    ...
    if friend_pairs:
        for (p1,p2) in friend_pairs:
            for t in tasks:
                for h in hours:
                    for j in days:
                        if x[p1,t,h,j].X > 0.5 and x[p2,t,h,j].X > 0.5:
