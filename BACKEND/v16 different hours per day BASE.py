...
def main(page: ft.Page):
    ...
    hours_per_day_st = {}          # {day_name: "08:00\n09:00\n..."}
    ...
    def build_hours_per_day(e=None):
        """Rebuild one TextField per day. New days get the default 08:00–17:00."""
        day_list = list(dict.fromkeys(x.strip() for x in tf_days.value.split("\n") if x.strip()))
        hours_col.controls.clear()
        page.update()
        hours_col.controls.append(ft.Text("Hours per Day", weight=ft.FontWeight.BOLD, size=12))
        for j in day_list:
            ...
            tf.on_change = _ch
            hours_col.controls.append(tf)
        page.update()

    tf_days.on_change = build_hours_per_day
    ...
    def build_avail():
        ...
        avail_ct.controls.clear(); page.update()
        ...
        avail_ct.controls.append(ft.Container(
        ...
        for j in days:
            ...
            avail_ct.controls.append(
                ft.Row([ft.Container(width=W_LBL + W_BTN + 4)] +
    ...
    def build_demand():
        people, tasks, hours, days = dims()
        demand_ct.controls.clear(); page.update()
        ...
        demand_ct.controls.append(ft.Container(
        ...
        for j in days:
            ...
            demand_ct.controls.append(hdr_row(day_hours, W_CELL))
            for t in tasks:
                cells = []
                for h in day_hours:
                    k = (t,h,j)
    ...
    def build_skills():
        people, tasks, hours, days = dims()
        skills_ct.controls.clear(); page.update()
        ...
        skills_ct.controls.append(ft.Container(
        ...
        for p in people:
            skills_ct.controls.append(
                ft.Row([plbl(p)] + [make_toggle(skills_st, (p,t), 1) for t in tasks], spacing=2))
        page.update()

    def build_force():
        people, tasks, hours, days = dims()
        force_ct.controls.clear(); page.update()
        for t in tasks:
            for j in days:
                day_hours = hours[j]
                force_ct.controls.append(ft.Text(f"-- {t} / {j} --", weight=ft.FontWeight.BOLD, size=14))
                force_ct.controls.append(hdr_row(day_hours))
                for p in people:
                    force_ct.controls.append(
                        ft.Row([plbl(p)] +
                               [make_toggle(force_st, (p,t,h,j), 0) for h in day_hours], spacing=2))
                force_ct.controls.append(ft.Divider())
        page.update()

    def build_social():
        people, tasks, hours, days = dims()
        social_ct.controls.clear(); page.update()
        ...
        social_ct.controls.append(ft.Container(
        ...
        for i, p1 in enumerate(people):
            ...
            if cells:
                social_ct.controls.append(ft.Row([plbl(p1)] + cells, spacing=2))
        page.update()

    def build_quota():
        people, tasks, hours, days = dims()
        quota_ct.controls.clear(); page.update()
    ...
    def build_rotation():
        people, tasks, hours, days = dims()
        rotation_ct.controls.clear(); page.update()
        for t in tasks:
    ...
    def build_weights():
        ...
        weights_ct.controls.clear(); page.update()
        header = ft.Column([
        ...
        for i, key in enumerate(weights_order):
            ...
            card = ft.Container(
                ...
                border=ft.border.all(1, ft.Colors.GREY_500), border_radius=8,
    ...
    def build_params():
        params_ct.controls.clear(); page.update()
    ...
    builders = {
        ...
        4: build_force, 5: build_social, 6: build_quota,
        7: build_rotation, 8: build_weights, 9: build_params}
    ...
    def build_output_grid(sol, people, tasks, hours, days, availability, emergency):
        ...
        for j in days:
            ...
            hdr = [ft.Container(
                ...
                width=NW, height=Ch, bgcolor="#455A64", alignment=ft.alignment.center,
                border=ft.border.all(1, "#37474F"))]
            for h in day_hours:
                hdr.append(ft.Container(
                    ft.Text(h, size=11, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                    width=CW, height=Ch, bgcolor="#455A64", alignment=ft.alignment.center,
                    border=ft.border.all(1, "#37474F")))
            hdr.append(ft.Container(
                ft.Text("Total", size=11, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                width=TW, height=Ch, bgcolor="#455A64", alignment=ft.alignment.center,
                border=ft.border.all(1, "#37474F")))
            ...
            for idx_p, p in enumerate(people):
                ...
                cells = [ft.Container(
                    ...
                    padding=ft.padding.only(left=8), border=ft.border.all(1, "#455A64"))]
                total = 0
                for h in day_hours:
        ...
        for p in people:
            output_ct.controls.append(ft.Text(f"  {p}: {workload.get(p, 0):.0f} hours", size=12))
        ...
        if rot_issues:
            ...
            output_ct.controls.append(ft.Text("  No rotation tasks defined.", size=12, italic=True))
        ...
        if cap_issues:
            ...
            output_ct.controls.append(ft.Text("  No captains designated or all hours covered.", size=12, italic=True))
        ...
        if emerg_issues:
            ...
            output_ct.controls.append(ft.Text("  None -- no emergency hours used!", size=12, italic=True))
    ...
    def do_solve(e):
        ...
        tabs.selected_index = 10; page.update()
    ...
    def do_stop(e):
        if running_model_ref[0]:
            output_ct.controls.insert(1, ft.Text(
                ...
                color=ft.Colors.ORANGE_500, italic=True))
            page.update()
    ...
    solve_btn = ft.Container(
        ...
        bgcolor=ft.Colors.BLUE_600, padding=10, border_radius=8,
