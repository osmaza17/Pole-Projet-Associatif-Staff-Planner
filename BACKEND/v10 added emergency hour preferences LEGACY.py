...
def main(page: ft.Page):
    ...
    def make_avail_toggle(sd, key, default=1):
        ...
        val = sd[key]
        btn = ft.ElevatedButton(
            _avail_lbl[val], width=W_BTN, height=30, data=key,
            bgcolor=_avail_clr[val],
            color=ft.Colors.WHITE, style=ft.ButtonStyle(padding=0))
        def _click(e, _sd=sd, _k=key):
            _sd[_k] = _avail_next[_sd[_k]]
            nv = _sd[_k]
            e.control.text = _avail_lbl[nv]
    ...
    def build_avail():
        ...
        for j in days:
            ...
            for p in people:
                avail_ct.controls.append(ft.Row([plbl(p)] + [make_avail_toggle(avail_st,(p,h,j), 1) for h in hours], spacing=2))
        ...
        for j in days:
            ...
            for idx_p,p in enumerate(people):
                ...
                for h in hours:
                    ...
                    # Thin border color based on availability state
                    if avail == 0: brd = ft.border.all(0.7, UNAVAIL_COLOR)
                    elif griev == 1: brd = ft.border.all(0.7, EMERG_COLOR)
                    else: brd = ft.border.all(0.7, AVAIL_COLOR)
        ...
        for p in people:
            for h in hours:
                for j in days:
                    ...
                    # 1=available(A=1,G=0), 0=unavailable(A=0,G=0), 2=emergency(A=1,G=1)
