...
def solve_model(data, ui_update_callback=None, active_model_ref=None):
    ...
    for loop_idx, current_days in enumerate(days_iterator):
        ...
        try:
            ...
            # x: main assignment (always created)
            x_set = {(p, t, h, d)
            ...
            # m: missing demand — ALWAYS created (structural slack)
            m_keys = [(t, h, d)
            ...
            # q: quota shortfall — ALWAYS created (structural slack)
            quota_keys = []
            for p in people:
                for t in tasks:
            ...
            # enemies_scope: ALWAYS computed — hard_enemies is a feasibility constraint
            ...
            # A. Demand coverage (always — m is structural slack)
            model.addConstrs(
            ...
            # D. Anti-ubiquity (always — structural)
            model.addConstrs(
            ...
            if W_EQ_GLOBAL > 0:
                if day_heuristics == 0:
                    model.addConstrs(
                        ...
                        name="eq_global_max")
                    model.addConstrs(
                        (x.sum(p, '*', '*', '*') >= w_min for p in people_available_in_day),
                        name="eq_global_min")
                else:
            ...
            # Hard enemies: ALWAYS enforced regardless of W_SOCIAL
            if hard_enemies:
                model.addConstrs(
            ...
            # I. Minimum quota (always — q is structural slack)
            quota_constr_keys = []
        ...
        # Al final del finally, añadir:
        finally:
            model.dispose()
            env.dispose()
            if active_model_ref is not None:
                active_model_ref[0] = None  # ← Limpiar referencia al modelo ya destruido
    ...
    # Work block fragmentation — computed from the assignment grid so it
    # works regardless of whether g variables existed in the model.
    gaps = []
    for d in days:
        day_gaps = []
        for p in people:
