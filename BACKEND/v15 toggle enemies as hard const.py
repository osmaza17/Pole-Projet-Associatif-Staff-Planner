...
def solve_model(data, ui_update_callback=None, active_model_ref=None):
    ...
    x_set = {(p,t,h,j) for p in people for t in tasks
                   for h in hours for j in days
                   if (p,t) in skill_set and (p,h,j) in avail_set}

    h_next = {hours[i]: hours[i+1] for i in range(len(hours) - 1)}
    ...
    m = model.addVars(tasks, hours, days, lb=0, vtype=GRB.CONTINUOUS, name="m")
    ...
    r_keys = [(p,h,j) for p in people for h in hours for j in days
              if (p,h,j) in avail_set]
    r = model.addVars(r_keys, lb=0, vtype=GRB.CONTINUOUS, name="r")

    # --- Equity bounds ------------------------------------------------------
    n_max = model.addVars(days, lb=0, ub=len(hours), vtype=GRB.CONTINUOUS, name="n_max")
    n_min = model.addVars(days, lb=0, ub=len(hours), vtype=GRB.CONTINUOUS, name="n_min")
    z_max = model.addVar(lb=0, ub=len(hours) * len(days), vtype=GRB.CONTINUOUS, name="z_max")
    z_min = model.addVar(lb=0, ub=len(hours) * len(days), vtype=GRB.CONTINUOUS, name="z_min")

    # --- Consecutive penalty: rotation=1 AND both hours have valid x --------
    consec_keys = [(p,t,h,j) for p in people for t in tasks
                   for h in hours[:-1] for j in days
                   if rotation.get(t, 0) == 1
                   and (p,t,h,j) in x_set
                   and (p,t,h_next[h],j) in x_set]
    ...
    active_hj = []
    if captains:
        active_hj = [(h,j) for h in hours for j in days
    ...
    for j in days:
        people_avail_j = [p for p in people
                          if any((p,h,j) in avail_set for h in hours)]
    ...
    people_with_avail = [p for p in people
                         if any((p,h,j) in avail_set for h in hours for j in days)]
    ...
    model.addConstrs(
        (x[p,t,h,j] + x[p,t,h_next[h],j] - c[p,t,h,j] <= 1
    ...
    workload = {}
    for p in people:
        total_hrs = sum(1 for t in tasks for h in hours for j in days
                        if (p,t,h,j) in x_set and x[p,t,h,j].X > 0.5)
            ...
            if rotation.get(t, 0) == 1:
                for j in days:
                    for i in range(len(hours) - 1):
                        h_curr = hours[i]; h_nxt = h_next[h_curr]
...
def main(page: ft.Page):
    ...
    tf_people = ft.TextField(
        value="Arnaud\nNina\nJoseph\nChloé\nNiels\nZiad\nTristan\nJasmin\nMarine\nNoé\nJules R\nGuillaume\nNoémie\nStanislas\nTérence\nManon In\nManon L\nPauline\nLucie\nDarius\nMattia\nPierre\nBaptiste B\nVincent\nMadeleine\nIlhan\nMatteo\nAlexandre L\nPablo\nJenaya\nLiv\nFaustin\nKenza\nJuliette B\nSarah\nAlexandre B\nRémi\nGabi\nJeanne B\nMatthieu A\nInès\nMaxime N\nAriane\nMatthias", multiline=True, min_lines=8, max_lines=200, label="People (one per line)", expand=True)
    tf_tasks = ft.TextField(
        value="Pénélope\nAugustin B\nAgathe\nRafael\nLuna\nLéna\nCamille\nJuliette M\nPaul\nManon P\nMatthieu G\nAlix\nNadim\nJean-Louis\nArthur", multiline=True, min_lines=8, max_lines=200, label="Tasks (one per line)", expand=True)
    ...
    def plbl(name, w=W_LBL):
        ...
        return ft.Container(
            ft.Text(name, size=11, no_wrap=True, weight=ft.FontWeight.BOLD if is_cap else None, color=CAPTAIN_BG if is_cap else None),
            width=w,
        )
    ...
    def build_avail():
        ...
        avail_ct.controls.append(ft.Container(content=ft.Text("Random Avail", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD), bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4, on_click=_rand_avail, width=150, alignment=ft.alignment.center))
            ...
            for p in people:
                def _make_row_toggle(_p=p, _j=j):
                    def _click(e):
                        ...
                        for _h in hours: avail_st[(_p, _h, _j)] = new_val
                        build_avail()
                    return ft.Container(content=ft.Text("row", color=ft.Colors.WHITE, size=12), width=W_BTN, height=30, bgcolor=ft.Colors.BLUE_GREY_400, alignment=ft.alignment.center, border_radius=4, on_click=_click)
    ...
    def build_demand():
        ...
        def _rand_demand(e):
            for t in tasks:
                for h in hours:
                    for j in days: demand_st[(t,h,j)] = str(random.randint(0, 4))
            build_demand()
            
        demand_ct.controls.append(ft.Container(content=ft.Text("Random Demand", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD), bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4, on_click=_rand_demand, width=150, alignment=ft.alignment.center))
    ...
    def build_skills():
        ...
        skills_ct.controls.append(ft.Container(content=ft.Text("Random Skills", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD), bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4, on_click=_rand_skills, width=150, alignment=ft.alignment.center))
    ...
    def build_social():
        ...
        social_ct.controls.append(ft.Container(content=ft.Text("Random Social", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD), bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4, on_click=_rand_social, width=150, alignment=ft.alignment.center))
        ...
        social_ct.controls.append(ft.Row([sw_hard_enemies], spacing=20))
        ...
        for i, p1 in enumerate(people):
            ...
            for p2 in people[1:]:
                ...
                if j2 > i:
                    ...
                    btn = ft.Container(content=ft.Text(_map_lbl[social_st[k]], color=ft.Colors.WHITE, size=14, weight=ft.FontWeight.BOLD), width=70, height=30, data=k, bgcolor=_map_clr[social_st[k]], alignment=ft.alignment.center, border_radius=4, on_click=_click)
                    cells.append(btn)
    ...
    def build_weights():
        ...
        for i, key in enumerate(weights_order):
            ...
            card = ft.Container(
                ...
                padding=10, bgcolor=ft.Colors.BLUE if weights_enabled[key] else ft.Colors.GREY_700,
                border=ft.border.all(1, ft.Colors.GREY_500), border_radius=8, margin=ft.margin.only(bottom=5)
            )
            items_controls.append(card)

        r_list = ft.ReorderableListView(controls=items_controls, on_reorder=handle_reorder)
        layout = ft.Column(controls=[header, ft.Container(content=r_list, width=420, height=500)], width=420, alignment=ft.MainAxisAlignment.START, horizontal_alignment=ft.CrossAxisAlignment.CENTER)
    ...
    def build_output_grid(sol, people, tasks, hours, days, availability, emergency):
        ...
        output_ct.controls.append(ft.Text(f"Status: {status}", weight=ft.FontWeight.BOLD, size=16))
            ...
            for idx_p,p in enumerate(people):
                ...
                for h in hours:
                    ...
                    if task: 
                        ...
                        cell = ft.Container(ft.Text(task, size=11, weight=ft.FontWeight.BOLD, color=fg, text_align=ft.TextAlign.CENTER), width=CW, height=Ch,bgcolor=bg, alignment=ft.alignment.center, border=brd, border_radius=4)
                    else: 
        ...
        for p in people: output_ct.controls.append(ft.Text(f"  {p}: {workload.get(p, 0):.0f} hours", size=12))
        output_ct.controls.append(ft.Text(f"  Global range: max={sol.get('z_max', 0):.0f}, min={sol.get('z_min', 0):.0f}", size=12, italic=True))
        ...
        if not sol.get("enforced_rest", False):
            ...
            if gaps:
                for line in gaps: output_ct.controls.append(ft.Text(f"  {line}", size=12))
            else: output_ct.controls.append(ft.Text("  Single block shifts! (Perfect)", size=12, italic=True))

        output_ct.controls.append(ft.Text("ROTATION FATIGUE", weight=ft.FontWeight.BOLD, size=14))
        if rot_issues:
            for line in rot_issues: output_ct.controls.append(ft.Text(f"  {line}", size=12))
        else: output_ct.controls.append(ft.Text("  No rotation tasks defined.", size=12, italic=True))
            
        output_ct.controls.append(ft.Text("SOCIAL", weight=ft.FontWeight.BOLD, size=14))
        if soc_issues:
            for line in soc_issues: output_ct.controls.append(ft.Text(f"  {line}", size=12))
        ...
        if cap_issues:
            for line in cap_issues: output_ct.controls.append(ft.Text(f"  {line}", size=12))
        else: output_ct.controls.append(ft.Text("  No captains designated or all hours covered.", size=12, italic=True))
        
        output_ct.controls.append(ft.Text("EMERGENCY CALL-INS", weight=ft.FontWeight.BOLD, size=14))
        if emerg_issues:
            for line in emerg_issues: output_ct.controls.append(ft.Text(f"  {line}", size=12))
