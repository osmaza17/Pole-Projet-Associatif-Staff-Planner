...
def solve_model(data, ui_update_callback=None, active_model_ref=None):
    ...
    # max_consec_hours: dict {person: int} — only people with a defined limit.
    # Empty dict means no enforced-rest constraint for anyone.
    ...
    for loop_idx, current_days in enumerate(days_iterator):
        ...
        # L. Per-person enforced rest: sliding window of max_consec_hours[p]
        for p_rest, limit in max_consec_hours.items():
            ...
            for d in current_days:
                ...
                if not any((p_rest, h, d) in avail_set_day for h in hours[d]):
                    continue
                model.addConstrs(
                    (gp.quicksum(x.sum(p_rest, '*', tau, d)
                                 for tau in hours[d][i:i + limit + 1]) <= limit
