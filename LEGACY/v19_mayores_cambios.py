...
from solve_model_2 import solve_model
...
def main(page: ft.Page):
    ...
    def _update_solve_blocked():
        has_errors = len(validation_errors["demand"]) > 0 or len(validation_errors["quota"]) > 0
    ...
    solve_btn_ref = [None]  # will be set when solve_btn is created
    ...
    def build_rotation_list(e=None):
        ...
        for t in tasks:
            ...
            val = rotation_st[t]
            btn = ft.Container(
                content=ft.Text("Rot" if val else "—", color=ft.Colors.BLACK, size=12, weight=ft.FontWeight.BOLD),
    ...
    def _scrollable_tab(ct):
        """Wrap a Column in a container with always-visible horizontal + vertical scrollbars."""
        return ft.Container(
            content=ft.Row(
                controls=[ct],
    ...
    def make_toggle(sd, key, default):
        ...
        def _click(e, _sd=sd, _k=key):
            ...
            e.control.update()
        return ft.Container(
            content=ft.Text(str(sd[key]), color=ft.Colors.WHITE, size=11, weight=ft.FontWeight.BOLD),
    ...
    def make_avail_toggle(sd, key, default=1):
        ...
        def _click(e, _sd=sd, _k=key):
            ...
            e.control.update()
        return ft.Container(
            content=ft.Text(_avail_lbl[sd[key]], color=ft.Colors.WHITE, size=11, weight=ft.FontWeight.BOLD),
    ...
    def hdr_row(labels, w=None):
        ...
        return ft.Row([ft.Container(width=W_LBL)] +
                      [ft.Container(
                       ...
                       for l in labels], spacing=2, wrap=False)
    ...
    def build_avail():
        ...
        def _on_day_change(e):
            avail_filter_st[0] = e.control.value
            build_avail()
        ...
        dd_day = ft.Dropdown(
            label="Select Day", value=avail_filter_st[0],
            options=[ft.dropdown.Option(d) for d in days],
            width=200, on_change=_on_day_change
        )
        ...
        buf.append(ft.Row([
            ...
            ft.Container(
                content=ft.Text("Random Avail (All Days)", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD, size=12),
        ...
        buf.append(ft.Row(
            ...
            [ft.Container(
             ...
             for l in day_hours], spacing=2, wrap=False))
        ...
        for p in people:
            ...
            buf.append(ft.Row(
                [plbl(p), row_btn] +
                [make_avail_toggle(avail_st, (p,h,j), 1) for h in day_hours], spacing=2, wrap=False))
    ...
    def build_demand():
        ...
        def _on_day_change(e):
            demand_filter_st[0] = e.control.value
            build_demand()
        ...
        dd_day = ft.Dropdown(
            label="Select Day", value=demand_filter_st[0],
            options=[ft.dropdown.Option(d) for d in days],
            width=200, on_change=_on_day_change
        )
        ...
        buf.append(ft.Row([
            ...
            ft.Container(
                content=ft.Text("Set All to 0", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD, size=12),
                bgcolor=ft.Colors.ORANGE_700, padding=10, border_radius=4,
                on_click=_zero_demand, width=150, alignment=ft.alignment.center),
            ft.Container(
                content=ft.Text("Random Demand (All Days)", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD, size=12),
                bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4,
        ...
        for t in tasks:
            ...
            for h in day_hours:
                ...
                tf = ft.TextField(value=demand_st[k], width=W_CELL, height=H_TF, text_size=11,
                ...
                def _ch(e, _k=k, _err=demand_error_text):
                    ...
                    has_err = len(validation_errors["demand"]) > 0
                    _err.value = f"⚠ {len(validation_errors['demand'])} cell(s) have invalid values. Only non-negative integers allowed." if has_err else ""
    ...
    def build_skills():
        if not _needs_rebuild("skills"): return
        people, tasks, hours, days = dims()
        ...
        buf = []
        buf.append(ft.Text("Skills Matrix", weight=ft.FontWeight.BOLD, size=16))
        buf.append(ft.Row([
            make_reset_btn("Reset Skills", _reset_skills),
            ft.Container(
                content=ft.Text("Random Skills", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD, size=12),
                bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4,
                on_click=_rand_skills, width=150, alignment=ft.alignment.center)
        ], spacing=10))
        buf.append(hdr_row(tasks, 70))
        for p in people:
            buf.append(ft.Row([plbl(p)] + [make_toggle(skills_st, (p,t), 1) for t in tasks], spacing=2))

        skills_ct.controls = buf
        page.update()

    def build_quota():
        if not _needs_rebuild("quota"): return
        people, tasks, hours, days = dims()
        ...
        buf = []
        buf.append(ft.Text("Minimum Quota Matrix", weight=ft.FontWeight.BOLD, size=16))
        quota_error_text = ft.Text("", color=ft.Colors.RED_400, size=12, visible=False)
        buf.append(ft.Row([
            make_reset_btn("Reset Quota", _reset_quota),
            ft.Container(
                content=ft.Text("Random Quota", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD, size=12),
                bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4,
                on_click=_rand_quota, width=150, alignment=ft.alignment.center),
        ], spacing=10))
        buf.append(quota_error_text)
        buf.append(hdr_row(tasks))
        for p in people:
            cells = []
            for t in tasks:
                k = (p, t)
                if k not in quota_st: quota_st[k] = ""
                is_valid = _validate_nonneg_int(quota_st[k])
                tf = ft.TextField(value=quota_st[k], width=W_CELL, height=H_TF, text_size=11,
                                  data=k, content_padding=ft.padding.all(2),
                                  border_color=ft.Colors.RED_400 if not is_valid else None)
                def _ch(e, _k=k, _err=quota_error_text):
                    ...
                    if _validate_nonneg_int(e.control.value):
                        ...
                        e.control.border_color = ft.Colors.RED_400
                    has_err = len(validation_errors["quota"]) > 0
                    _err.value = f"⚠ {len(validation_errors['quota'])} cell(s) have invalid values. Only non-negative integers allowed." if has_err else ""
                    _err.visible = has_err
                    _update_solve_blocked()
                    e.control.update()
                    _err.update()
    ...
    def build_force():
        people, tasks, hours, days = dims()
        if not days or not tasks: return

        if force_filter_st[0] not in days: force_filter_st[0] = days[0]
        if force_filter_st[1] not in tasks: force_filter_st[1] = tasks[0]

        def _on_day_change(e):
            force_filter_st[0] = e.control.value
            build_force()

        def _on_task_change(e):
            force_filter_st[1] = e.control.value
            build_force()
        ...
        dd_day = ft.Dropdown(
            label="Select Day", value=force_filter_st[0],
            options=[ft.dropdown.Option(d) for d in days],
            width=150, on_change=_on_day_change
        )
        dd_task = ft.Dropdown(
            label="Select Task", value=force_filter_st[1],
            options=[ft.dropdown.Option(t) for t in tasks],
            width=200, on_change=_on_task_change
        )
        ...
        for p in people:
            buf.append(ft.Row(
                [plbl(p)] + [make_force_toggle(force_st, (p,t,h,j), 0, task_bg, task_fg) for h in day_hours], spacing=2, wrap=False))
    ...
    def build_social():
        ...
        buf.append(ft.Row([
            ...
            ft.Container(
                content=ft.Text("Random Social", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD, size=12),
                bgcolor=ft.Colors.PURPLE_400, padding=10, border_radius=4,
            ...
            for p2 in people[1:]:
                ...
                if j2 > i:
                    ...
                    def _click(e, _k=k):
                        ...
                        e.control.update()
                    btn = ft.Container(
                        content=ft.Text(_map_lbl[social_st[k]], color=ft.Colors.WHITE, size=12,
    ...
    def build_output_grid(sol, people, tasks, hours, days, availability, emergency):
        ...
        for j in days:
            ...
            buf.append(ft.Row(legend_items, spacing=8, wrap=False))
            buf.append(ft.Divider())
    ...
    SIDEBAR_HOVER_BG = "#455A64"
    SIDEBAR_TEXT_COLOR = "#ECEFF1"
    SIDEBAR_SELECTED_TEXT = "#4FC3F7"

    # Menu items definition: (label, icon, index)
    menu_items_def = [
    ...
    dim_tab_content = ft.Container(
        content=ft.Row(
            controls=[tf_people, captains_col, tf_tasks, rotation_col, tf_days, hours_col, right_controls_col],
            spacing=20,
            alignment=ft.MainAxisAlignment.START,
            vertical_alignment=ft.CrossAxisAlignment.START),
    ...
    def switch_page(idx):
        ...
        for i, btn_container in enumerate(menu_btn_refs):
            if i == idx:
                btn_container.bgcolor = SIDEBAR_SELECTED_BG
                btn_container.content.controls[0].color = SIDEBAR_SELECTED_TEXT
                btn_container.content.controls[1].color = SIDEBAR_SELECTED_TEXT
                btn_container.content.controls[1].weight = ft.FontWeight.BOLD
            else:
                btn_container.bgcolor = None
                btn_container.content.controls[0].color = SIDEBAR_TEXT_COLOR
                btn_container.content.controls[1].color = SIDEBAR_TEXT_COLOR
                btn_container.content.controls[1].weight = None
        if idx in builders:
            with ui_lock:
                builders[idx]()
        page.update()
    ...
    # Build sidebar menu buttons
    for label, icon, idx in menu_items_def:
        is_selected = (idx == 0)
        btn = ft.Container(
            content=ft.Row([
                ft.Icon(icon, color=SIDEBAR_SELECTED_TEXT if is_selected else SIDEBAR_TEXT_COLOR, size=20),
    ...
    # Solve / Stop buttons for sidebar header
    ...
    sidebar = ft.Container(
        ...
        content=ft.Column(
            controls=[
                ...
                # Solve / Stop buttons
                ft.Container(
                    content=ft.Column([solve_btn, stop_btn], spacing=6),
                    padding=ft.padding.only(bottom=8),
                ),
                ft.Divider(color="#455A64", height=1),
                # Menu items
                ft.Column(
                    controls=menu_btn_refs,
                    spacing=2,
