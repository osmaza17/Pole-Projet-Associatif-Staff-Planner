...
def solve_model(data, ui_update_callback=None, active_model_ref=None):
    ...
    for loop_idx, current_days in enumerate(days_iterator):
        ...
        # Solo se penalizan los reinicios tras un hueco (horas[d][1:] en adelante).
        # La primera hora NO se penaliza: trabajar un bloque continuo debe costar 0.
        obj += W_GAP * gp.quicksum(
            g[p, h, d]
            for p in people
            for d in current_days
            for h in hours[d][1:]          # ← excluye la primera hora del día
            if (p, h, d) in avail_set_day)
        ...
        #
        # IMPORTANTE: solo los reinicios (horas[1:]) se penalizan en el objetivo.
        # La primera hora (horas[0]) necesita la variable g para el constraint de
        # detección, pero NO se penaliza: trabajar un bloque continuo debe costar 0,
        # no 1. Sin este ajuste, el solver tiene incentivo a concentrar toda la carga
        # en el mínimo número de personas (cada trabajador extra añade 1 penalización
        # de gap aunque no tenga ningún hueco en su turno).
