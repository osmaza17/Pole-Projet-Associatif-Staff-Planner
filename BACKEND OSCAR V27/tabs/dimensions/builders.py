...
def _rank_tier_color(rank: int, total: int) -> str:
    """Badge color based on priority tier."""
    if rank == 0:
...
def build_rotation_list(state, rotation_col: ft.ReorderableListView,
    ...
    for rank, t in enumerate(tasks):
        ...
        drag_handle = ft.Icon(
            ft.Icons.DRAG_INDICATOR, size=_s(18),
            color=ft.Colors.GREY_500, tooltip="Drag to reorder priority")

        rank_badge = ft.Container(
            content=ft.Text(f"#{rank + 1}", size=_s(10),
                            weight=ft.FontWeight.BOLD,
                            color=ft.Colors.WHITE),
            bgcolor=_rank_tier_color(rank, N),
            padding=ft.padding.symmetric(horizontal=_s(6), vertical=_s(2)),
            border_radius=4, width=_s(34),
        ...
        dur_tf = ft.TextField(
            ...
            width=_s(55), text_size=_s(11),
            text_align=ft.TextAlign.CENTER,
            label="h", label_style=ft.TextStyle(size=_s(9)),
